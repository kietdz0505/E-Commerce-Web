...
function RegisterPopup({ open, onClose, onSwitchToLogin }) {
    ...
    const uploadAvatarToCloudinary = async () => {
        ...
        try {
            ...
            if (!cloudinaryUrl) {
                throw new Error('URL Cloudinary không được cấu hình.');
            }
            ...
            if (!res.ok) {
                ...
                throw new Error(errorText || 'Tải ảnh lên thất bại.');
            }
    ...
    const handleSubmit = async (e) => {
        ...
        if (!validatePhone(formData.phone)) {
            setError('Số điện thoại không hợp lệ (10-12 chữ số).');
            setIsLoading(false);
            return;
        }

        try {
            const picture = avatarFile ? await uploadAvatarToCloudinary() : '';
            ...
            const res = await fetch(getOAuthUrl('register'), {
                method: 'POST',
                ...
                <div className="modal-content rounded-4 shadow">
                    ...
                    <div className="modal-body pt-2">
                        <form className="d-flex flex-column gap-3 mb-3" onSubmit={handleSubmit}>
                            ...
                            />
                            {error && (
                                <div className="text-danger text-center small">{error}</div>
                            )}
                            <button
                                type="submit"
